...
const ConsumerFilters = ({ page, setPage, filters, setFilters }) => {
  ...
  const debouncedFetchName = useCallback(
    ...
    [page, filters.brandId, filters.status, filters.phone]
  )
  ...
  const debouncedFetchPhone = useCallback(
    ...
    [page, filters.brandId, filters.status, filters.name]
  )
  ...
  useEffect(() => {
    if (filters.name !== lastNameRef.current) {
      ...
      setPage(1)
      debouncedFetchName(filters.name)
    }
  }, [filters.name])
